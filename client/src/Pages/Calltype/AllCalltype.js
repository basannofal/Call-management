...
const AllCalltype = () => {
  ...
  const savedata = async (e) => {
    ...
    if (!callType) {
      ...
      try {
        await axios
          .post(`${process.env.REACT_APP_PROXY_URL}/addcalltype`, group)
          .then((res) => {
            window.alert("Call Type Added successful!");
  ...
  // Get Call type Data
  const [calltype, setCalltype] = useState([]);
  const [perCalltype, setPerCalltype] = useState([]);
  ...
  const getPerCalltype = async (id) => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_PROXY_URL}/getpercalltype/${id}`
      );
      setPerCalltype(res.data);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  // Trash (delete) Call type Records
  const trashCalltype = async (id) => {
    try {
      if (window.confirm("Are you sure you want to remove this Call Type")) {
        const res = await axios.delete(
          `${process.env.REACT_APP_PROXY_URL}/trashcalltype/${id}`
        );
        window.location.reload();
        getCalltype();
      }
  ...
  const filteredData = calltype.filter((item) => {
    return item.call_type.toLowerCase().includes(searchFilter);
  });
